...
import { getPaymentsByMonthYear, getActiveTenants } from "@/lib/firestore";
import { useLang } from "@/contexts/LangContext";
import { FileBarChart2, IndianRupee, CheckCircle2, Clock, Zap, Droplets } from "lucide-react";
  ...
  useEffect(() => {
    async function loadReport() {
      ...
      try {
        ...
        const totalRentCollected = rows.reduce((s, r) => s + (r.rentPaid ? Number(r.tenant?.rentAmount)||0 : 0), 0);
            ...
            <>
              ...
              <div className="rounded-3xl p-5 space-y-4" style={card}>
                ...
                {[
                  ...
                  { label: t("pending"),   val: data.totalRentPending,   cls: "text-brick-600 dark:text-brick-400" },
                ].map(row => (
                  <div key={row.label} className="flex items-center justify-between">
                    <span className={`text-base font-semibold ${row.cls}`}>{row.label}</span>
                      ...
                      <div key={row.id} className="rounded-3xl p-4" style={card}>
                        <div className="flex items-center justify-between mb-3">
                          ...
                          {row.rentPaid
                            ...
                            : <span className="flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-ember-100 dark:bg-ember-900 text-ember-700 dark:text-ember-300"><Clock size={11}/>{t("pending")}</span>
                          }
                        </div>
                        <div className="grid grid-cols-3 gap-2 text-sm">
                          {[
                            { l: t("rent"), v: `₹${Number(row.tenant?.rentAmount||0).toLocaleString("en-IN")}` },
