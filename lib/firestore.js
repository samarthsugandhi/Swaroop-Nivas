...
export async function getYearlyStats(year) {
  ...
  // Deduplicate: If multiple records exist for same tenant in same month, pick the Paid one
  const sortedPayments = paymentsSnap.docs
    .map(d => ({ id: d.id, ...d.data() }))
    .sort((a, b) => (b.rentPaid ? 1 : 0) - (a.rentPaid ? 1 : 0));
...
export async function cleanupDuplicates() {
  ...
  const groups = {};
  all.forEach(p => {
    const key = `${p.tenantId}_${p.month}_${p.year}`;
  ...
  Object.values(groups).forEach(list => {
    if (list.length > 1) {
      // Pick the winner: prioritize paid, then notes, then amount
      const sorted = [...list].sort((a, b) => {
        if (a.rentPaid !== b.rentPaid) return b.rentPaid ? 1 : -1;
        if (!!a.notes !== !!b.notes) return a.notes ? 1 : -1;
        if ((a.rentPaidAmount||0) !== (b.rentPaidAmount||0)) return (b.rentPaidAmount||0) - (a.rentPaidAmount||0);
        return 0;
      });
  ...
  const deletePromises = toDelete.map(id => deleteDoc(doc(db, "payments", id)));
